...
class TemplateDir {
    getIdFromLink(folderLink) {
    let id;
    id = folderLink.split(/[?\/]+/);
    this.id = id[id.length-2];
  };

  getTemplate() {
    try {
      this.folder = DriveApp.getFolderById(this.id);
